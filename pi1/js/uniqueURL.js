  ...
  if (typeof exports === "object" && exports) {
	...
	uniqueURL.constructor = function(url){
		...
		this.getParams = function(){
			...
			var res = {};
			var params = this.url.split(this.separator);
		...
		this.setUniqueURL = function(json){	    	  
			  ...
			  for (var i = 0, l = json.length; i < l; i++) {
			      ...
			      switch(json[i].key){
				  case "req":
					  uniqueURL = sprintf('%s=%s', json[i].key, Base64.encode(json[i].value));
					  break;
				  case "view":
				  case "category":
					  uniqueURL = sprintf('%s%s%s=%s', uniqueURL, this.separator, json[i].key, json[i].value);
					  break;
			      }		  
			  } 	  
